...
def train(training_dataloader, model, criterion, optimizer, device, batches_to_accumulate):
    ...
    for batch_idx, batch in enumerate(training_dataloader):
        X, y, num_graphs, bbox, prev_image, cur_image = \
...
def main(env_config_file:str, model_type:str, model_config_file:str, disease_ids:list):
    ...
    for id in disease_ids:
        disease_str += str(id)
    logging.basicConfig(filename=f'./logs_rebut/cxr_{disease_str}_{model_type}_{seed}.log', format='%(asctime)s %(message)s',
    ...
    training_dataset = GraphDataset(regions, train_file, image_path_file, bbox_file_orig, adjacency_file, mimic_data_dir, input_size, label_list, data_transforms)
    training_dataloader = DataLoader(training_dataset, batch_size=config['train_batch_size'], shuffle=True, num_workers=8)

    validation_dataset = GraphDataset(regions, valid_file, image_path_file, bbox_file_orig, adjacency_file, mimic_data_dir, input_size, label_list, data_transforms)
    validation_dataloader = DataLoader(validation_dataset, batch_size=config['test_batch_size'], shuffle=True, num_workers=8)
    ...
    print('Create Model')
    model = ChexRelNet(training_dataset.edge_index , config, regions)
    ...
    plt.savefig(f'curves/loss_curves_classes_{num_classes}_D_{disease_str}_M_{model_type}_{seed}.jpg', bbox_inches='tight', dpi=150)
    plt.close()
    ...
    plt.savefig(f'curves/acc_curves_classes_{num_classes}_D_{disease_str}_M_{model_type}_{seed}.jpg', bbox_inches='tight', dpi=150)
    plt.close()

    PATH = f"./model_checkpoint/best_checkpoint_{disease_str}_{model_type}_{seed}.pth"
    ...
    infer_df.to_csv(f'./inference/cxr_{disease_str}_{model_type}_{num_classes}_{seed}_infer_{test_acc}.csv')
    print(f'Stored inference results at ./inference/cxr_{disease_str}_{model_type}_{num_classes}_{seed}_infer_{test_acc}.csv')


def parse_args():
    parser = argparse.ArgumentParser(description="ChexRelNet Training Arguments")
